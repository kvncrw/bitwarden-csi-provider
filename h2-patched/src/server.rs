...
impl proto::Peer for Peer {
    ...
    */
    ...
    fn convert_poll_message(
        ...
        // header.
        //
        // PATCHED: Accept malformed :authority headers (e.g., filesystem paths
        // from gRPC clients connecting over Unix sockets). Fall back to
        // "localhost" if the authority can't be parsed as a valid URI authority.
        if let Some(authority) = pseudo.authority {
            let maybe_authority = uri::Authority::from_maybe_shared(authority.clone().into_inner());
            parts.authority = Some(maybe_authority.unwrap_or_else(|_| {
